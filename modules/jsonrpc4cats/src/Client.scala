      ...
      yield new RpcClient[F] {
        ...
        def request[A, B](
          ...
          for
            ...
            json = Request(
              endpoint.method,
              Some(toJson(params)(using endpoint.codecA)),
              reqId,
              headers,
            )
            ...
            case err: Response.Error =>
              Sync[F].pure[RpcResponse[B]](RpcFailure(endpoint.method, err))
            case suc: Response.Success =>
              Sync[F]
                .delay[RpcResponse[B]](RpcSuccess(readFromArray[B](suc.result.value), suc))
                .handleErrorWith { err =>
                  Sync[F]
                    .pure(RpcFailure(endpoint.method, Response.invalidParams(err.toString, reqId)))
                }
            }
          yield rpcResponse
        }
