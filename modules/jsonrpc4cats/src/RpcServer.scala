            ...
            responseFiber <- supervisor.supervise(
              handler
                ...
                .handleErrorWith {
                  ...
                  case None =>
                    Logger[F].error(s"Unhandled JSON-RPC error handler for request $request did not return a response").as(
        ...
        case Some(handler) =>
          for
            ...
            result <- response match
              case None => ().pure
              case Some(nonEmpty) =>
                Logger[F].error(s"Obtained non-empty response $nonEmpty for notification $notification!")
          yield
            ()
