...
object macros:
  def unfoldUnionType(
    ...
    list: collection.mutable.ArrayBuffer[quotes.reflect.TypeRepr] =
  ...
  def mapUnionTypeImp[T: Type, V: Type](casesExpr: Expr[Any => V])(using
    quotes: Quotes,
  ): Expr[Seq[V]] =
    ...
    val applications = Varargs(types.map { tpe =>
      typeMap
        ...
        .getOrElse(
          report.throwError(
            s"invalid type ${tpe.show}, expected one of: ${typeMap.map(_._1.show).mkString(", ")}",
          ),
        )
    })
    applications
