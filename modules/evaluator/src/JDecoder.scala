  ...
  extension (path: JDecoderPath)
    ...
    inline def expect[F[_]: Monad, T](ctx: EvaluationContext[F], expr: EvaluatedJValue[F]): F[T] =
      if isEmpty then
        ...
        ctx.expect[T](expr, s"error at path $toString, unexpected type ${EvaluationContext.typeString(EvaluationContext.theExpr)}, expected ${EvaluationContext.typeString[F, T]}")
...
object JDecoder:
    ...
    def decode(ctx: EvaluationContext[F], path: JDecoderPath, expr: EvaluatedJValue[F]): F[Path] =
      path.expect[F, String](ctx, expr).map { str =>
        if str.startsWith("/") then
          ...
          ctx.workspaceDir.resolve(str).normalize
      }

  given iterableDecoder[F[_]: Monad: Parallel, L[X] <: Iterable[X], T](using f: collection.Factory[T, L[T]], d: => JDecoder[F, T]): JDecoder[F, L[T]] with
  ...
  given mapDecoder[F[_]: Monad: Parallel, T](using => JDecoder[F, T]): JDecoder[F, Map[String, T]] with
    def decode(ctx: EvaluationContext[F], path: JDecoderPath, expr: EvaluatedJValue[F]): F[Map[String, T]] =
      for
