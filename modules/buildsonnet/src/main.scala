...
object Buildsonnet extends IOApp:
  override def run(args: List[String]): IO[ExitCode] =
    ...
    val uninterpreted =
      if args.contains("--") then args.dropWhile(_ != "--").drop(1)
      else List.empty

    CommandIOApp.run(
      name = "buildsonnet",
      header = """buildsonnet will search for a build.jsonnet file starting from the
  ...
  def main(interpreted: List[String], uninterpreted: List[String]): Opts[IO[ExitCode]] =
    Options.runCommand.map { (directory, bloopPort, logToConsole, logToFile) =>
      ...
      while currPath != null && buildFile == null do
        ...
        if Files.exists(tryPath) then buildFile = tryPath
        currPath = currPath.getParent

      val run =
        for
          uninterpreted <-
            if uninterpreted.isEmpty then
          ...
          buildFile <-
            if buildFile == null then
              IO.raiseError(
                new CmdlineError(
                  "could not find build.jsonnet file in current directory or in any parent directory",
                ),
              )
          ...
          buildObject <- Parser(sourceFile)
            ...
            .fold(
              { error =>
                ...
                IO.raiseError(
                  new CmdlineError(
                    s"syntax error at ${scala.Console.UNDERLINED}${sourceFile.path}${scala.Console.RESET}:$line:$col",
                  ),
                )
              },
              _.pure[IO],
                  ...
                  Seq(
                    EvaluatedJValue.JArray(
                      ...
                      IArray.unsafeFromArray(
                        buildArgs.map(EvaluatedJValue.JString(Source.empty, _)).toArray,
                      ),
                    ),
                  ),
                  Seq.empty,
