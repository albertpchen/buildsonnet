...
private class Std[F[_]: Async: ConsoleLogger: Logger: Parallel] private (
  ...
  extension [T](arg: Arg[T]) def default: Option[EvaluatedJValue[F]] = arg
        ...
        while error.isEmpty && i < paramsDef.size do
          ...
          if currPosArgs.nonEmpty && isGivenNamedArg then
            error = Some(
              params
                .ctx
                .error(params.src, s"both positional and named arg provided for argument $argName"),
            )
          else if currPosArgs.nonEmpty then
            locals(i) = currPosArgs.head
            currPosArgs = currPosArgs.tail
          else if isGivenNamedArg then locals(i) = argMap(argName)
          else if default.isDefined then locals(i) = default.get
  ...
  val members: F[Map[String, EvaluatedJValue[F]]] =
    for scalaProject <- LazyObjectValue(
        true,
        Sync[F].defer {
    ...
    yield
      val members = Map(
        "toString" -> function1(Arg.x)(x =>
          ctx.singleLinePrint(x).map(EvaluatedJValue.JString(src, _)),
        ),
        ...
        "uniq" -> function2(Arg.arr, Arg.keyF(jidentity)) { (arr, keyF) =>
          (
            ctx.expect[EvaluatedJValue.JFunction[F]](keyF),
            ctx.expect[EvaluatedJValue.JArray[F]](arr),
          ).mapN { (keyF, arr) =>
            val elements =
              if keyF eq jidentity then arr.elements.distinct
                ...
                arr
                  .elements
                  .distinctBy(e =>
                    keyF.fn(EvaluatedJValue.JFunctionParameters(src, ctx, Seq(e), Seq.empty)),
                  )
        ...
        "write" -> function2(Arg.pathName, Arg.contents) { (pathName, contents) =>
          (ctx.expect[String](pathName), ctx.expect[String](contents))
            ...
            .flatMap { (pathName, contents) =>
              val path =
                if pathName.startsWith("/") then Paths.get(pathName).normalize
                else ctx.workspaceDir.resolve(pathName).normalize
              ...
              else writeImpl(path, contents).map(p => EvaluatedJValue.JString(src, p.toString))
            }
        },
        "find" -> function3(
          Arg.dir,
          Arg.pattern,
          Arg.syntax(EvaluatedJValue.JString(stdSrc, "glob")),
        ) { (dir, pattern, syntax) =>
          (
            ...
            val dirPath =
              if dirName.startsWith("/") then Paths.get(dirName).normalize
              else ctx.workspaceDir.resolve(dirName).normalize
            if !java.nio.file.Files.exists(dirPath) then
              ...
              yield EvaluatedJValue.JArray(src, IArray.unsafeFromArray(elements))
          }
        },
        ...
        "join" -> function2(Arg.sep, Arg.arr) { (sep, arr) =>
          (ctx.expect[String](sep), ctx.expect[EvaluatedJValue.JArray[F]](arr))
            ...
            .flatMap {
              ...
              case (sep, arr) if arr.elements.size == 1 =>
                ctx.expect[EvaluatedJValue.JString[F]](arr.elements.head).widen
              case (sep, arr) =>
          ...
          ctx.expect[String](varName).flatMap { varName =>
            Sync[F].defer {
              try
                ...
                if value eq null then ctx.error(src, s"environment variable \"$varName\" not set")
                else EvaluatedJValue.JString(src, value).pure
              catch
                case e: java.lang.SecurityException =>
                  ctx.error(
                    src,
                    s"could not access environment variable \"$varName\": ${e.getMessage}",
                  )
            }
          }
        },
        "java" -> EvaluatedJValue
          .JObject
          .static(
            stdSrc,
            Map(
              ...
              "fetchJvm" -> function2(
                ...
                for
                  ...
                  result <- Async[F].fromFuture(future).flatMap {
                    ...
                    case Right(f) =>
                      EvaluatedJValue.JString[F](src, f.toPath.normalize.toString).pure
                  }
                yield result
              },
            ),
          ),
        "scala" -> EvaluatedJValue
          .JObject
          .static(
            stdSrc,
            Map(
              "Dep" -> function4(
                ...
                ).mapN { (org, name, version, crossVersion) =>
                  EvaluatedJValue
                    .JObject
                    .static(
                      stdSrc,
                      ...
                      ) ++ (if !crossVersion.isNull then Some("crossVersion" -> crossVersion)
                            else None),
                    )
                }
              },
              ...
              "compile" -> function1(Arg.project) { (project) =>
                for
                  ...
                  result <- response.foldValue(ctx.error(src, _))(_ =>
                    EvaluatedJValue.JNull[F](src).pure,
                  )
                yield result
              },
              "run" -> function5(
                ...
                (
                  ...
                  for
                    ...
                    result <- response
                      .foldValue(ctx.error(src, _))(_ => EvaluatedJValue.JNull[F](src).pure)
                  yield result
                }
              },
              "test" -> function4(
              ...
              "classpath" -> function1(Arg.project) { project =>
                for
                  ...
                  _ <- bloopServer
                    ...
                    .flatMap(_.foldValue(ctx.error(src, _)) { compileResult =>
                      compileResult.statusCode match
                      case bsp4s.StatusCode.Ok => ().pure
                      case bsp4s.StatusCode.Error =>
                        ctx.error(src, s"compilation for '${project.name}' failed")
                      case bsp4s.StatusCode.Cancelled =>
                        ctx.error(src, s"compilation for '${project.name}' was cancelled")
                    })
...
object Std:
  ...
  private case class CtxImpl[F[_]: Async: Parallel: ConsoleLogger: Logger](
    ...
    def `import`(src: Source, filename: String): F[EvaluatedJValue[F]] =
      for
        importFile <- Sync[F].defer {
          ...
          val importFile =
            if currFileParent eq null then Paths.get(filename)
            else currFileParent.resolve(filename)
          if importFile == currFileParent then error(src, s"file $importFile imports itself")
          else if !importFile.toFile.exists then
            error(src, s"imported file does not exist $importFile")
          else importFile.pure
        }
            ...
            Parser(srcFile)
              ...
              .fold(
                error => {
                  ...
                  this.error(
                    src,
                    s"syntax error at ${Console.UNDERLINED}${srcFile.path}${Console.RESET}:$line:$col",
                  )
                },
                ast =>
                  importCtx.flatMap { importCtx =>
    ...
    def importStr(src: Source, filename: String): F[EvaluatedJValue.JString[F]] =
      for
        ...
        contents <- Sync[F].delay(
          scala.io.Source.fromFile(importStrFile.toFile).getLines.mkString("\n"),
        )
  ...
  def apply[F[_]: Async: ConsoleLogger: Logger: Parallel](
    ...
    for
      ...
      bloopServerEither <- lazyResource(
        SocketConnection
          .connectToLauncher[F](
