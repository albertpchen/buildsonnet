...
object Parser:
  def toIArray[T: reflect.ClassTag](l: List[T]): IArray[T] =
    val res = Array.ofDim(l.size)
    var curr = l
    var i = 0
    while curr.nonEmpty do
      ...
      curr = curr.tail
      i += 1
    return IArray.unsafeFromArray(res)
...
final class Parser(file: SourceFile):
  ...
  val exprBase: P[JValue] = {
    ...
    val function = (keyword("function") *> __ *> (params <* __) ~ exprDefer).withRange.map {
      case (range, (params, body)) => JFunction(range, Parser.toIArray(params), body)
    }
    val ifExpr = (
  ...
  def parseFile: Either[P.Error, JValue] =
