...
object SourceFile:
  ...
  private class BinTree(
    ...
    def find(i: Int): BinTree =
      if value < i then
        if right.isEmpty then this
        else
          val foundRight = right.get.find(i)
          if foundRight.value < i then
            if (i - value) < (i - foundRight.value) then this
            else foundRight
          else this
      else if value > i then
        if left.isEmpty then this
        else left.get.find(i)
      else this
  private def makeBinTree(
    ...
    val span = end - begin
    if span == 1 then new BinTree(begin, ints(begin), None, None)
    else if span == 2 then
      new BinTree(
        begin,
        ints(begin),
...
enum JObjMember:
  ...
  def setFile(file: SourceFile): JObjMember =
    ...
    case v: JField =>
      v.copy(src = v.src.setFile(file), key = v.key.setFile(file), value = v.value.setFile(file))
    case v: JAssert =>
      v.copy(
        src = v.src.setFile(file),
        cond = v.cond.setFile(file),
        msg = v.msg.map(_.setFile(file)),
      )
...
object JObjMember:
  ...
  given ToExpr[JObjMember] with
    def apply(member: JObjMember)(using Quotes): Expr[JObjMember] =
      member match
      case JLocal(src, name, value) =>
        '{ JLocal(${ Expr(src) }, ${ Expr(name) }, ${ Expr(value) }) }
      case JField(src, key, plus, isHidden, value) =>
        '{
          JField(
            ${ Expr(src) },
            ${ Expr(key) },
...
enum JValue extends HasSource:
  ...
  case JSlice(
    ...
    loc: JValue,
    index: JValue,
    endIndex: Option[JValue],
    stride: Option[JValue],
  )
  case JApply(
    src: Source,
    loc: JValue,
    positionalArgs: List[JValue],
    namedArgs: List[(String, JValue)],
  )
  ...
  def setFile(file: SourceFile): JValue =
    ...
    case v: JObjectComprehension =>
      v.copy(
        src = v.src.setFile(file),
        locals = v.locals.map(JObjMember.setFile(_, file)),
    ...
    case v: JIndex =>
      v.copy(src = v.src.setFile(file), loc = v.loc.setFile(file), index = v.index.setFile(file))
    case v: JSlice =>
      v.copy(
        src = v.src.setFile(file),
...
object JValue:
  ...
  def ofIArray[T](xs: Seq[Expr[T]])(using Type[T])(using Quotes): Expr[IArray[T]] =
    if xs.isEmpty then '{ IArray.empty[T](using scala.compiletime.summonInline[ClassTag[T]]) }
    else '{ IArray(${ Varargs(xs) }: _*)(using scala.compiletime.summonInline[ClassTag[T]]) }

  def jparamsExpr(xs: JParamList)(using Quotes): Expr[JParamList] =
    if xs.isEmpty then '{ IArray.empty[(String, Option[JValue])] }
    else
  ...
  given ToExpr[JValue] with
    def apply(jvalue: JValue)(using Quotes): Expr[JValue] =
      ...
      case JArray(src, elements) =>
        '{ JArray(${ Expr(src) }, ${ ofIArray(elements.map(Expr(_))) }) }
      case JObject(src, members) =>
        '{ JObject(${ Expr(src) }, ${ Expr.ofList(members.map(Expr(_))) }) }
      case JObjectComprehension(src, locals, key, value, forVar, inExpr, cond) =>
        '{
          JObjectComprehension(
            ${ Expr(src) },
            ${ Expr.ofList(locals.map(Expr(_))) },
            ${ Expr(key) },
            ${ Expr(value) },
            ${ Expr(forVar) },
            ${ Expr(inExpr) },
      ...
      case JGetField(src, loc, field) =>
        '{ JGetField(${ Expr(src) }, ${ Expr(loc) }, ${ Expr(field) }) }
      case JIndex(src, loc, index) => '{ JIndex(${ Expr(src) }, ${ Expr(loc) }, ${ Expr(index) }) }
      case JSlice(src, loc, index, endIndex, stride) =>
        '{
          JSlice(
            ${ Expr(src) },
            ${ Expr(loc) },
      ...
      case JApply(src, loc, positionalArgs, namedArgs) =>
        '{
          JApply(
            ${ Expr(src) },
            ${ Expr(loc) },
            ${ Expr.ofList(positionalArgs.map(Expr(_))) },
            ${
              Expr.ofList(namedArgs.map { (name, arg) =>
                '{ ${ Expr(name) } -> ${ Expr(arg) } }
              })
            },
          )
        }
      case JBinaryOp(src, left, op, right) =>
        '{ JBinaryOp(${ Expr(src) }, ${ Expr(left) }, ${ Expr(op) }, ${ Expr(right) }) }
      case JUnaryOp(src, op, expr) => '{ JUnaryOp(${ Expr(src) }, ${ Expr(op) }, ${ Expr(expr) }) }
      case JLocal(src, name, value, result) =>
        '{ JLocal(${ Expr(src) }, ${ Expr(name) }, ${ Expr(value) }, ${ Expr(result) }) }
      case JFunction(src, params, body) =>
        '{ JFunction(${ Expr(src) }, ${ jparamsExpr(params) }, ${ Expr(body) }) }
      case JIf(src, cond, trueValue, elseValue) =>
        '{ JIf(${ Expr(src) }, ${ Expr(cond) }, ${ Expr(trueValue) }, ${ Expr(elseValue) }) }
      case JError(src, expr) => '{ JError(${ Expr(src) }, ${ Expr(expr) }) }
      case JAssert(src, cond, msg, expr) =>
        '{ JAssert(${ Expr(src) }, ${ Expr(cond) }, ${ Expr(msg) }, ${ Expr(expr) }) }
  ...
  private def readFileString(relativeFilename: Expr[String])(using quotes: Quotes): String =
    ...
    if currPath == null then
      report.throwError(
        "could not find file of macro expansion location",
...
type JParamList = IArray[(String, Option[JValue])]
